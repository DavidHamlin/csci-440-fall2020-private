...
public class Homework4 extends DBTest {
    ...
    public void selectPopularTracksAndTheirAlbums() throws SQLException {
        ...
        List<Map<String, Object>> albums = executeSQL("SELECT DISTINCT albums.AlbumId FROM tracks JOIN invoice_items ON tracks.TrackId = invoice_items.TrackId JOIN albums ON tracks.AlbumId = albums.AlbumId GROUP BY tracks.TrackId HAVING COUNT (tracks.TrackId)>1");
...
//         HINT: join to invoice items and do a group by/having to get the right answer
//        List<Map<String, Object>> tracks = executeSQL("SELECT DISTINCT email" +
//                " FROM customers" +
...
//                "                  WHERE (employees.LastName = 'Peacock')" );
        ...
        List<Map<String, Object>> tracks = executeSQL("SELECT DISTINCT Email " +
                ...
                "WHERE (customers.SupportRepId IN (SELECT EmployeeId " +
                "       FROM employees " +
                "       WHERE employees.LastName = 'Peacock')) AND (tracks.GenreId IN (SELECT genres.GenreId " +
                "       FROM genres WHERE genres.Name = 'Rock'))");
